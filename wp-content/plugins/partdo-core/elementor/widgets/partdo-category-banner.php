...
class Partdo_Category_Banner_Widget extends Widget_Base {
    use Partdo_Helper;
	
    public function get_name() {
        return 'partdo-category-banner';
    }
	...
	protected function register_controls() {
		...
		$this->add_control( 'banner_type',
			[
				...
				'options' => [
					...
					'type3'	 	  => esc_html__( 'Style 3', 'partdo-core' ),
				],
			]
		);
		...
		$this->partdo_query_elementor_controls($post_count = 8, $column = 4);
		...
		$defaultimage = plugins_url( 'images/banner-04.jpg', __DIR__ );
        ...
        $this->add_control( 'banner_desc',
            [
                ...
                'type' => Controls_Manager::TEXTAREA,
                'default' => 'Nis telesa, laber. Mytonomi bedessade miheten. Pokura rengen, lulurat. Niren nunade håd. Sest berade. ',
                'description'=> 'Add a description.',
				'label_block' => true,
				'condition' => ['banner_type' => ['type1','type3','select-type']]
            ]
        );	

		$this->add_control( 'percentage',
            [
                'label' => esc_html__( 'Percentage', 'partdo-core' ),
		...
		$this->add_control( 'percentage_heading',
            [
                'label' => esc_html__( 'PERCENTAGE', 'partdo-core' ),
                'type' => Controls_Manager::HEADING,
				'separator' => 'before',
		...
		$this->add_control( 'percentage_bg_color',
           [
			   ...
			   'condition' => ['banner_type' => ['type2','type3']]
           ]
        );
		
		$this->add_control( 'percentage_color',
			[
               'label' => esc_html__( 'Percentage Color', 'partdo-core' ),
               'type' => Controls_Manager::COLOR,
               'default' => '',
               'selectors' => ['{{WRAPPER}} .entry-sale-percentage , {{WRAPPER}} .klbth-banner .badge' => 'color: {{VALUE}};'],
			]
        );
		
		$this->add_control( 'percentage_opacity_important_style',
            [
                ...
                'step' => 0.1,
                'default' => '',
                'selectors' => ['{{WRAPPER}} .entry-sale-percentage , {{WRAPPER}} .klbth-banner .badge ' => 'opacity: {{VALUE}} ;'],
            ]
        );
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .entry-sale-percentage , {{WRAPPER}} .klbth-banner .badge ',
			]
		);
		...
		$this->add_control( 'button_color',
			[
               ...
               'type' => Controls_Manager::COLOR,
               'default' => '',
               'selectors' => ['{{WRAPPER}} .entry-footer .btn' => 'color: {{VALUE}};']
			]
        );
		...
		$this->add_control( 'button_opacity_important_style',
            [
                ...
                'step' => 0.1,
                'default' => '',
                'selectors' => ['{{WRAPPER}} .entry-footer .btn' => 'opacity: {{VALUE}} ;']
            ]
        );
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .entry-footer .btn',
			]
		);
	...
	protected function render() {
		...
		if($settings['banner_type'] == 'type3'){
			...
			$output .= '<div class="entry-wrapper overlay-25-dark-max768 dump">';
			$output .= '<div class="entry-inner w-50">';
			$output .= '<div class="entry-heading banner-heading"><span class="badge">'.esc_html($settings['banner_subtitle']).'</span>';
			$output .= '<h2 class="entry-title font-banner-sm">'.esc_html($settings['banner_title']).' </h2>';
			$output .= '</div>';
			if($settings['banner_btn_title']){
				$output .= '<div class="entry-footer vertical banner-footer">';
				$output .= '<div class="banner-button">';
			...
			$output .= '</div>';
			$output .= '</div>';
			$output .= '<div class="entry-media overlay-15-dark-min768"><img src="'.esc_url($settings['image']['url']).'" alt=""/></div><a class="link-mask" href="'.esc_url($settings['banner_btn_link']['url']).'" '.esc_attr($target.$nofollow).'> </a>';
			$output .= '</div>';
			$output .= '</div>';
		} else {
			$output .= '<div class="block-column banner-column"> ';
			$output .= '<div class="klbth-banner style-inner color-scheme-'.esc_attr($colortype).' space-sm align-start justify-start hover-zoom">';
			$output .= '<div class="entry-wrapper overlay-25-dark-max768 dump">';
			$output .= '<div class="entry-inner w-100">';
			$output .= '<div class="entry-heading banner-heading">';
			if($settings['percentage']){
				$output .= '<div class="entry-sale-percentage">'.esc_html($settings['percentage']).'<span>%</span></div>';
			}
			$output .= '<div class="entry-small">'.esc_html($settings['banner_subtitle']).'</div>';
			$output .= '<h2 class="entry-title font-banner-sm">'.esc_html($settings['banner_title']).'</h2>';
			$output .= '</div>';
			$output .= '<div class="entry-excerpt font-xs weight-300">';
			$output .= '<p>'.esc_html($settings['banner_desc']).'</p>';
			$output .= '</div>';
			if($settings['banner_btn_title']){
				$output .= '<div class="entry-footer vertical banner-footer">';
