...
## Scripts
*************************************************/
function partdo_products_navigation_scripts() {
	wp_register_style( 'klb-products-navigation',   plugins_url( 'css/single-products-navigation.css', __FILE__ ), false, '1.0');
}
add_action( 'wp_enqueue_scripts', 'partdo_products_navigation_scripts' );

/*************************************************
## Single Product Nav
*************************************************/ 

if ( ! function_exists( 'partdo_product_nav' ) ) {
	function partdo_product_nav() {
						...
						<div class="product-short">
							<div class="product-short-image">
								<a href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>" class="product-thumb">
									<?php echo apply_filters( 'partdo_products_nav_image', get_the_post_thumbnail( $prev_post, 'thumbnail' ) ); ?>
								</a>
							</div>
						...
						<div class="product-short">
							<div class="product-short-image">
								<a href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>" class="product-thumb">
									<?php echo apply_filters( 'partdo_products_nav_image', get_the_post_thumbnail( $next_post, 'thumbnail' ) ); ?>
								</a>
							</div>
							<div class="product-short-description">
								<h3 class="product-title">
...
add_action( 'klb_woocommerce_after_breadcrumb', 'partdo_product_nav', 2 );