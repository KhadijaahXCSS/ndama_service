...
## Scripts
*************************************************/
function partdo_people_added_in_cart_scripts() {
	wp_enqueue_style( 'klb-people-added-in-cart',   plugins_url( 'css/people-added-in-cart.css', __FILE__ ), false, '1.0');
}
add_action( 'wp_enqueue_scripts', 'partdo_people_added_in_cart_scripts' );

/*************************************************
## People Have this in their carts.
*************************************************/
if( ! function_exists( 'partdo_people_added_in_cart' ) ) {
	function partdo_people_added_in_cart(){
		global $wpdb, $product;
		$in_basket = 0;
		...
		if( $in_basket ){
			...
			echo '<p>'.esc_html__('This product has been added to','partdo-core').' <strong>'.sprintf( esc_html__( '%d  people\'s', 'partdo-core' ), $in_basket ).'</strong>'.esc_html__('carts.','partdo-core').'</p>';
			echo '</div>';
		}
