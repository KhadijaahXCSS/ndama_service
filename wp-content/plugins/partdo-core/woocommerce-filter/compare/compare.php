	...
	class KlbCompare {
		...
		function init() {
			...
			self::$fields = apply_filters( 'klbcp_fields', [
				'remove'             => '&nbsp;',
				'image'             => esc_html__( 'Image', 'partdo-core' ),
				'name'             	=> esc_html__( 'Name', 'partdo-core' ),
				'sku'               => esc_html__( 'SKU', 'partdo-core' ),
				'rating'            => esc_html__( 'Rating', 'partdo-core' ),
				'price'             => esc_html__( 'Price', 'partdo-core' ),
				'stock'             => esc_html__( 'Stock', 'partdo-core' ),
				'weight'            => esc_html__( 'Weight', 'partdo-core' ),
				'add_to_cart'       => esc_html__( 'Add to cart', 'partdo-core' ),
			] );
			...
			add_shortcode( 'klbcp_list', [ $this, 'partdo_compare_list' ] );

			// add button for partdo_compare_action - product box
			add_action( 'partdo_compare_action', [ $this, 'partdo_compare_button' ], 11 );
			
			// add button for single
			add_action( 'partdo_single_compare', [$this,'partdo_compare_button'], 40);
		}
		...
		function enqueue_scripts() {
			...
			wp_localize_script( 'klbcp-frontend', 'klbcp_vars', [
					...
					'view_page_text'     => esc_html__( 'View Compare Page', 'partdo-core' ),
					'close_text'     	 => esc_html__( 'Close', 'partdo-core' ),
					'message_added'      => esc_html__( '{name} has been added to Compare list.', 'partdo-core' ),
					'message_removed'    => esc_html__( '{name} has been removed from the Compare list.', 'partdo-core' ),
					'message_exists'     => esc_html__( '{name} is already in the Compare list.', 'partdo-core' ),
					'limit'              => 100,
					'limit_notice'       => esc_html__( 'You can add a maximum of {limit} products to the comparison table.', 'partdo-core' ),
					'button_text'        => apply_filters( 'klbcp_button_text', esc_html__( 'Compare', 'partdo-core' ) ),
					'button_text_added'  => apply_filters( 'klbcp_button_text_added', esc_html__( 'Compare', 'partdo-core' ) ),
				]
			);
		}
			...
			if ( is_array( $products ) && ( count( $products ) > 0 ) ) {
				...
				foreach ( $products as $product_id ) {
					...
					foreach ( $saved_fields as $saved_field ) {
						switch ( $saved_field ) {
							case 'remove':
								//remove button
								$products_data[ $product_id ]['remove'] = ' <span class="klbcp-remove" data-id="' . $product_id . '"><i class="klbth-icon-xmark"></i> ' . esc_html__( 'Remove', 'partdo-core' ) . '</span>';
				...
				$table = '<div class="klbcp-no-result">
							<div class="cart-empty-page">
								...
								<p class="cart-empty">'.esc_html__( 'The comparison table is empty.', 'partdo-core' ).'</p>
								<p class="return-to-shop">
									<a class="button wc-backward'.esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ).'" href="'.esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ).'">
										'.esc_html( apply_filters( 'woocommerce_return_to_shop_text', esc_html__( 'Return to shop', 'partdo-core' ) ) ).'
									</a>
								</p>
							</div>
		...
		function partdo_compare_button( $attrs ) {
			...
			if ( $attr_id ) {
				// button text
				$text = esc_html__( 'Compare', 'partdo-core' );
		...
		function partdo_compare_list( $attrs ) {
		...
		function account_items( $items ) {
			...
			if ( ! isset( $items['compare'] ) ) {
				$items['compare'] = apply_filters( 'klbcp_myaccount_compare_label', esc_html__( 'Compare', 'partdo-core' ) );
			}
		...
		public static function get_page_url() {
			$page_id  = get_theme_mod('partdo_compare_page');
