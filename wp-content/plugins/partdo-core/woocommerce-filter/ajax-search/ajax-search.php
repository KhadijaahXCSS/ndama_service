...
## Ajax Search Scripts
*************************************************/ 
function partdo_ajax_search_scripts() {
	wp_enqueue_style( 'partdo-ajax-search',    plugins_url( 'css/ajax-search.css', __FILE__ ), false, '1.0');
	wp_enqueue_script( 'partdo-ajax-search',    plugins_url( 'js/ajax-search.js', __FILE__ ), false, '1.0');
	wp_localize_script( 'partdo-ajax-search', 'partdosearch', array(
		'ajaxurl' => esc_url(admin_url( 'admin-ajax.php' )),
	));
}
add_action( 'wp_enqueue_scripts', 'partdo_ajax_search_scripts' );


/*************************************************
## Ajax Login CallBack
*************************************************/ 
add_action( 'wp_ajax_nopriv_ajax_search', 'partdo_ajax_search_callback' );
add_action( 'wp_ajax_ajax_search', 'partdo_ajax_search_callback' );
function partdo_ajax_search_callback() {
	global $wpdb;
	...
	if ( $args->have_posts() ) {
		...
		if($args->found_posts > 7){
			...
			echo '<a href="'.esc_url($searchall).'"><span>'.esc_html__('See all products...','partdo-core').'</span> <span>('.esc_html($args->found_posts).')</span></a>';
			echo '</li>';
		}
		...
		echo '<ul><li><span>'.sprintf(esc_html__( 'No results found for "%s"', 'partdo-core' ), esc_html( $keyword )).'</span></li></ul>';
	}
...
*************************************************/ 
if ( ! function_exists( 'partdo_sku_search_query' ) ) {
	function partdo_sku_search_query( $where, $s ) {
		global $wpdb;
...
add_filter( 'posts_search', 'partdo_product_search_sku', 9 );
if ( ! function_exists( 'partdo_product_search_sku' ) ) {
	function partdo_product_search_sku( $where, $class = false ) {
		...
		return partdo_sku_search_query( $where, $s );
